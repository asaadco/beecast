...
# NN_MODEL = None
NN_MODEL = './rl_server/results/nn_model_ep_3300.ckpt'
...
#########
#   Assumptions:
#   1. We already have IP addresses formed in a textfile stored on the server-side
#   2. The group records contains LAST_REQUEST, IP_1, LAST_REQUEST_1, ..... IP_N, LAST_REQUEST_N
    # Assume you have two IP addresses for now
#########
...
############################################
    ...
    ########################################   
                    ...
                    # if(post_data["Group"] == 1):
                    #     groupRequest = self.group("1")
                    #     if(int(groupRequest) < post_data['lastRequest']):
                    #         print "Updating Group Last Requested Chunk"
                    #         filename = GROUP_FOLDER+"/1.txt"
                    #         with open(filename, 'wb') as group_file:
                    #             group_file.write(str(post_data['lastRequest']))
                    #     else:
                    #         self.send_header("lastRequest", int(groupRequest)) 
        ...
        def group(self, group):
            filename = GROUP_FOLDER+"/"+group+".txt"
            print "=============="
            print self.client_address
            with open(filename, 'r') as group_file:
                return group_file.read()
